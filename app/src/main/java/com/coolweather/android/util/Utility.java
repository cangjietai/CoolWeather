...
/**
 * 一个用来解析和处理JSON格式数据的工具类
 * 处理方式：
 * 1，先使用JSONArray和JSONObject将数据解析出来，然后组装成实体类对象
 * 2，再调用save()方法将数据存储到数据库中
 */
public class Utility {
        ...
        if (!TextUtils.isEmpty(response)) {
            try {
                ...
                for (int i=0; i<allProvinces.length(); i++) {
                    JSONObject provinceJSONObject = allProvinces.getJSONObject(i);
    ...
    public static Weather handleWeatherResponse(String response) {
        try {
            ...
            //fromJson()方法能直接将JSON数据转换为Weather对象
